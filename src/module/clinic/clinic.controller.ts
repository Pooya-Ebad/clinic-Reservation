...
export class clinicController {
  ...
  @Post("register_step1")
  @ApiOperation({
    summary: "clinic signup section 1",
    description: "you need to login as a doctor first",
  })
  @ApiResponse({
    status: 201,
  ...
  Register(@Body() createClinicDto: CreateClinicDto, @Req() request: Request) {
  ...
  @Post("register_step2")
  @ApiOperation({
    summary: "clinic signup section 2",
    description: "you need to login as a doctor first",
  })
  @ApiResponse({
    status: 201,
  ...
  createDocument(
      ...
      new ParseFilePipe({
        validators: [
          ...
          new FileTypeValidator({
            fileType: /(image\/(png|jpg|jpeg)|application\/(pdf|docx))/,
          }),
        ],
    ...
    return this.clinicService.CreateDocument(
      clinicDocumentDto,
      files,
      request.clinic
    );
  ...
  async find(
    ...
    const result = await this.clinicService.findClinic(
      paginationDto,
      searchDto
    );
    if (result.clinic.length == 0)
  ...
  getAppointments(@Param() Param: GetAppointmentsDto) {
  ...
  @ApiOperation({
    summary: "you can accept or reject clinics",
  ...
  AddDoctor(@Body() Doctor_License: LicenseNumberDto, @Req() request: Request) {
    const { Medical_License_number } = Doctor_License;
    return this.clinicService.addDoctor(
