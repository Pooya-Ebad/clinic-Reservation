...
export class SearchService {
  ...
  async search(searchDto: GlobalSearchDto, paginationDto: PaginationDto) {
    ...
    const { clinic, pagination: clinicsPagination } =
      await this.clinicsService.findClinic(paginationDto, {
        search,
    ...
    return {
      pagination: PaginationGenerator(
        page,
        limit,
        clinicsPagination.total_count + doctorsPagination.total_count
      ),
